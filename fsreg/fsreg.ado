...
program define fsreg
   ...
   syntax varlist(numeric min=2) [if] [in], 
      CLEAR [Dots SAVing(str) REPLACE ID(varname) Level(int $S_level) RESiduals RSTUdent RSTAndard NRes(int 0) INITial(int 0) OBS(numlist integer) LMD(passthru) WHICH MEDian]
   ;
   #delimit cr
   ...
   * So, the first piece
   ...
   if "`which'"~="" { 
     ...
     di in gre "Started with observations: " in yel "`stlist'"
   }
...
****** di in whi "`resstr'"
   ...
   * This is the list of the t-stats. Do not forget the constant !
   ...
   qui postfile FSREG _nobs _added `*' _cons `tlist' _sigma2 _scom2 _scom1 _scom05 _sco0 _sco05 _sco1 _sco2 `resstr' using `saving', `replace' every(20)
** di in whi `"POSTFILE was: FSREG _nobs _added `*' _cons `tlist' _sigma2 _scom2 _scom1 _scom05 _sco0 _sco05 _sco1 _sco2 `resstr' using `saving', `replace' "'
...
******** di in whi `"POST: FSREG `m' (`added') `betas' `tstats' (`s2') `scolist' `resstr' "'
         post FSREG `m' (`added') `betas' `tstats' (`s2') `scolist' `resstr' 
